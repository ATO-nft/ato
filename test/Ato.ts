...
describe("Ato", function () {
  ...
  async function deployContractsFixture() {
    ...
    const atoContract = new ethers.Contract(ato.address, atoAbi, issuer);

    return { issuer, bob, alice, ato, atoContract, name, symbol, uri };
  }

  describe("Deployment", function () {
    it("Should deploy Ato.sol", async function () {
      const { atoContract, issuer } = await loadFixture(deployContractsFixture);
      expect(await atoContract.owner()).to.equal(issuer.address);
    });
  });

  describe("Transfers", function () {
    it("Should transfer the NFT to Bob", async function () {
      const { ato, atoContract, issuer, bob } = await loadFixture(deployContractsFixture);
